/**
 * Phase 1A-16 / 1A-17: 原価費目マスター (CostCategory) シードの共通モジュール
 *
 ...
 *
 * 呼び出し側:
 * - scripts/seed-cost-categories.ts        (dev 専用エントリ、dev でないと abort)
 * - scripts/seed-cost-categories-prod.ts   (本番専用エントリ、三重ガード付き)
 */
import {
  PrismaClient,
...
export const LV1: readonly Lv1Row[] = [
...
export const LV2: readonly Lv2Row[] = [
  // parent=MATERIAL (9)
 ...
 * Lv1 → Lv2 の順で冪等に投入する。
 *
 * 戻り値: `{ created, skipped }` (合計 39 行に対する内訳)
 */
export async function seedCostCategories(
        ...
        await tx.auditLog.create({
          data: {
            ...
            afterData: {
              ...
              isSystemReserved: true,
              seedScript: "seeds/cost-categories-core.ts",
            },
          },
        })
        created++
  ...
  // 最終検証 (logging のみ。caller の判定材料にはしない)
