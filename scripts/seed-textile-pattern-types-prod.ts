#!/usr/bin/env tsx
/**
 * Phase 1A-13d: 柄種別マスター (TextilePatternType) シード — **本番専用エントリ**
 *
 * 三重ガード (seed-cost-categories-prod.ts / seed-colors-prod.ts と同形):
 *   1. 明示フラグ: 環境変数 CONFIRM_PROD_SEED=TEXTILE_PATTERN_TYPE_9 を要求
 ...
 *      (非 TTY 環境では abort)
 *
 * 投入ロジック自体は scripts/seeds/textile-pattern-types-core.ts と共通。
 *
 * 使い方 (本番に投入する場合のみ):
 *   DATABASE_URL=<本番 URL> CONFIRM_PROD_SEED=TEXTILE_PATTERN_TYPE_9 \
 *     npx tsx scripts/seed-textile-pattern-types-prod.ts
 */
import { createInterface } from "node:readline/promises"
import { PrismaClient } from "@prisma/client"
import {
  seedTextilePatternTypes,
  PATTERN_TYPES,
} from "./seeds/textile-pattern-types-core"

const REQUIRED_PROD_HOST = "shuttle.proxy.rlwy.net:16099"
const REQUIRED_FLAG = "TEXTILE_PATTERN_TYPE_9"
...
async function main() {
  ...
  const banner = [
    ...
    `投入予定   : TextilePatternType ${PATTERN_TYPES.length} 件 (SOLID/BD/ST/CK/DT/PR/AO/ML/OT)`,
    `冪等       : 既存があればスキップ (companyId + typeCode)`,
    "本当に本番へ投入しますか? (yes/no): ",
  ].join("\n")
  ...
  // ─── 投入実行 ───
  console.log(
    `\n[seed-textile-pattern-types | prod] 三重ガード通過。投入を開始します。`,
  )
...
main()
  .catch((e) => {
    console.error("[seed-textile-pattern-types | prod] FATAL:", e)
    process.exit(1)
  })
