...
| B-170 | 未着手 | クライアント名寄せ基盤。先方の商品コード（26S-JK01）と自社品番（AOI-26SS-M-TS-001）は別体系で現在は人が突き合わせている。①品番×クライアントの対応表 ②色名の対応表 ③対応が付かない行を人が紐づける画面。★Sku.clientSkuCode は既存列だが SKU 単位では粒度が細かすぎる（実データの先方コードは品番単位・色とサイズは別列）。★B-171 と B-149 の共通前提。これが無いと どちらの経路も1行も登録できない | sales-order-spec-addendum-v0_1-2026-08-19.md §1（ライフサイクル: 6 → 7） |
