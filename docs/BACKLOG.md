...
| B-082 | 未着手 | 品番一覧サムネ拡大(a)/絵型メイン+サムネ方式(b)。★2026-09-05: 案b（絵型メインの2段方式）で確定。40px→96px級。統合仕様確認書 docs/specs/b-082-b-090-b-091-product-list-spec-confirmation-v1_0-2026-09-05.md | — |
...
| B-190 | 未着手 | 品番カルテ一覧で ARCHIVED を既定で非表示にする。気づきメモ原文「品番カルテデフォルトでアーカイブを非表示にしたい」。★recon 実測（2026-09-05）: listProducts の where は companyId と deletedAt:null のみが固定で status は任意条件のため、既定で ARCHIVED が一覧に混在している。★where に固定除外を足さず status フィルタの既定値で実現する（隠すのであって消さない）。docs/specs/b-082-b-090-b-091-product-list-spec-confirmation-v1_0-2026-09-05.md P-7 で確定・統合 PR に含む。★2026-08-22 の気づきメモ読み込みで拾い漏れていた項目 | 気づきメモ（〜2026-08-19） |
| B-191 | 未着手 | 品番の文字列と現在の属性（カテゴリ／シーズン／ブランド）の食い違いを一覧で可視化する。★採番済みの productCode は categoryId を変更しても変わらない（products.ts:679 のコメント）。品番の文字列は「発番時」の属性、列は「現在」の属性であり両者は食い違いうる。食い違う行にだけ小さくバッジを出し、一致する行には何も出さない。docs/specs/b-082-b-090-b-091-product-list-spec-confirmation-v1_0-2026-09-05.md P-5 で確定・統合 PR に含む | 2026-09-05 の設計時に発見（B-090 の派生） |
