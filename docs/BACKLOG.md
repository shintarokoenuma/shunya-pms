...
| B-192 | 未着手 | 受注の編集で「各 SKU に歩留まり（率 or 枚数）を入力してください」が出て保存できない。★2026-09-06 に dev で観測: SO-2026-0002（1品番）は保存できたが SO-2026-0001（2品番）は保存が止まった。エラー画面には受注数が空の SKU 行が多数並んでおり、それらにも歩留まりの入力が要求されている。★実装の現況（grep のみ・未 recon）: validators/sales-order.ts:104-122 が yieldMode 既定 QUANTITY で yieldQuantity null を superRefine で却下し、sales-order-form.tsx:332-336 が同条件で toast を出す。form:229 は「各 SKU の歩留まりに既定（加算枚数0）を敷く。編集復元済みの値は保持する」としており、既定を敷いているのに止まった点が未解明。★原因の候補は2つ（①旧行の yieldMode=null が「復元済み」として保持され既定が敷かれない ②2品番目以降のブロックに既定が配られない）。どちらかは未確定のため、着手時に recon してから直す。★B-166（受注入力フォームの入力設計・受注数0の行が大量に並ぶ）と同じ画面の問題だが、あちらは「埋もれる」、本件は「保存できない」で症状が違う。★PR #139 とは無関係の既存の穴 | 2026-09-06 dev で観測 |
