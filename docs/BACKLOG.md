...
| B-091 | 未着手 | ピクトグラム/アイコンでの視認性向上（DRAFT/量産/PO/WO等）。★2026-09-05 追記: グレーが「完了ステータス」と「休眠機能（B-132）」で二重の意味を持つ問題を本項に含める。色の体系そのものは B-188。★2026-09-05 追記2: 状態を色ではなく形で区別する実装例（白丸/半分塗り/黒丸/二重丸に!/点線・遅延行は左端の太罫）が docs/REFERENCE_INDEX.md に記載の叩き台 HTML にある。作法のみ流用可・業務モデルは別案件 | — |
