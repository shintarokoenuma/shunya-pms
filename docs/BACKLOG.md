...
| B-171 | 未着手 | 専用オーダーページ（クライアントが自ら入力する画面）。★受注の本命の入口。オーダーシートの様式がクライアントごとに全て異なり、取込パーサーがクライアント数だけ必要になるため、入口を揃えるほうが安いと判断（慎太郎さん確認 2026-08-19）。先方の画面には先方の商品コードを表示する必要があるため B-170 が前提。認証は B-172 | sales-order-spec-addendum-v0_1-2026-08-19.md §1（ライフサイクル: 6. 受注期間 → 7） |
