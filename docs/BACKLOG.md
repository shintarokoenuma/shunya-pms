...
| B-182 | 未着手 | 伝票詳細から品番カルテへ戻る動線の欠落。src/lib/nav/breadcrumb.ts は「根は常に品番カルテ」を純関数として実装済みだが、画面内の明示リンクがこれに従っていない。実測（main 3f5b8f1・186エッジ）で、受注（SO）と納品（DLV）は品番カルテにもサンプルにも向かうエッジが1本も無く、発注（PO/WO）はサンプルへは戻れるが品番へは戻れない。サンプルに紐づかない量産発注では画面内の戻り先が消える。★受注は PR-1（2026-08-17）で新設された最も新しい画面で、背骨への接続を通していない可能性が高い。★パンくず自体は breadcrumb.ts により表示されるため、追加すべきは詳細ページ内の明示リンク | docs/b-173-flow-map-2026-08-29.md §4-2（ライフサイクル: 横断（UI）） |
...
| B-187 | 未着手 | 日本語フォント未指定の解消。Geist/Inter とも subsets:["latin"] で日本語グリフを持たず、日本語は全て OS フォールバック任せ。Windows で游ゴシック Regular に落ちて細く潰れる（スタッフの多数派が Windows）。--font-sans=Geist と body=Inter の二重指定、--font-mono が未定義の --font-geist-mono を参照している点、--border が薄すぎる点も併せて解消。2ファイルのみ・migration なし | — |
