...
| B-090 | 未着手 | カルテ一覧の2段表示化（1段目絵型大/2段目情報）。★2026-09-05 追記: カテゴリ列が categoryCode（例 M-TS）のみで意味が読めない。ProductCategory.categoryName（日本語・NOT NULL）は既に存在し、CategorySummary に name を足せば表示できる（listProducts の select 拡張のみ）。B-082（サムネ拡大）・B-091 と統合設計する | — |
| B-091 | 未着手 | ピクトグラム/アイコンでの視認性向上（DRAFT/量産/PO/WO等）。★2026-09-05 追記: グレーが「完了ステータス」と「休眠機能（B-132）」で二重の意味を持つ問題を本項に含める。色の体系そのものは B-188 | — |
