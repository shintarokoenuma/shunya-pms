...
| B-167 | 未着手 | SoItem.unitPrice の nullable 化（migration 1本・DROP NOT NULL・非破壊）＋金額の突合警告。希望数量の段階では最終コストが出ていない実務があるため単価必須にできない（慎太郎さん確認 2026-08-19）。0 と未定を区別したうえで 0 は異常値として警告。★明細から再計算した金額と先方提示額の乖離を警告する。実オーダーシートに消費税の二重計上（正 11,515,350 に対し 21,983,850）を確認済み＝先方提示額を信用しない | sales-order-spec-addendum-v0_1-2026-08-19.md §4（ライフサイクル: 7. 受注確定） |
| B-168 | 未着手 | productionQuantity の正を SO 由来に一本化する。現在 Sku.productionQuantity（数量マトリクス下段でクリック手入力可）と SoItem.productionQuantity の2箇所があり、どちらが正か未定義。原設計 §2.6「量産発注=受注合計＋歩留まり率5%」に従い SO 由来を正とし、Sku 側の手入力を廃止する。歩留まり率の適用箇所もここで決める。★B-148 PR-2 の前提。決着させずに PR-2 に進まない | 慎太郎さん確認 2026-08-19（ライフサイクル: 7 → 8. 量産発注） |
