...
| B-173 | 未着手 | 動線マップの作成（コード変更なし・docs のみ）。13ステップのライフサイクル × 実装済み画面の対応表と、画面間の遷移を1枚にまとめ、欠けている遷移・重複した入口を洗い出す。★実例: ステータス変更 UI の所在が分からない／カルテの「受注を作成」と既存伝票の編集のどちらが主か読めない／見積もりから発注先を指定する動線の有無が分からない。★B-090〜B-093・B-166 の優先順位付けの前提。画面が増えるほど作成が重くなるため早期に着手する | 慎太郎さん確認 2026-08-19（ライフサイクル: 全体） |
