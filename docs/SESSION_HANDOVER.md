...
★★最初の作業は下表ではなく ⑨ のスキル修正。コード変更なしで 5〜10分。
誤った実例が入ったままだと、このメモを読んだ次セッションが誤った教訓を適用する。
⑨ を片付けてから下表の順に入ること。
...
## ⑨ スキルの修正（★次セッションの最初の作業・コード変更なし）

file-write-verification は 2026-09-06 に更新して保存済み。
★ただし2箇所が誤った前提で書かれている。**次セッションの冒頭で差し替えて再提案すること**
（B-192 の修正より先。コード変更なし・5〜10分）。

手順:

    1. propose_skills は SKILL.md の全文置き換えになるので、まず現物を全文読む
       （~/.claude/skills/synced/ 配下の file-write-verification/SKILL.md。
        パスは環境で変わるので find で探す）
    2. 下の2箇所だけを差し替え、他は1文字も変えずに propose_skills で再提案する
    3. 保存は慎太郎さんが提案カードで行う。効くのはそのまた次のチャットから

★正しく入っているので触らないもの: 鉄則5-1 の4例目（2026-09-06・見出し数 26 と書いたが実測 27 の件）。
「検証の期待値を、そのセッションで自分が変更した対象の『変更前の値』から取らない。
期待値に数を書くなら、その数をどこで測ったかを1行添える」。これはそのまま残す。

★誤っているので差し替えるもの（2箇所）:

    (1) 鉄則4 の末尾に足した「★自分が『仮説』と明記して出したものを
        『確定』『原因判明』に格上げしない」の項
    (2) 鉄則8 の「★報告に書かれた『検証済み』は…」節に足した 2026-09-06 / B-192 の実例段落

どちらも「B-192 の recon は一度も実行されていないのに、報告で確定に格上げされた」という前提で
書かれている。**これは事実ではない。** recon は実行済みで、再実行させたところ dev DB の実測
（SO-2026-0001 は2行とも yield_mode=NULL）とコード3段が揃い、原因は確定していた。

差し替える文（この趣旨で書く。文言は整えてよい）:

    ★raw 出力がこの会話に届いていないことを、「実行されていない」の根拠にしない（2026-09-06 追加）。

    実例（2026-09-06 / shunya-pms / B-192）: claude.ai 側が出した recon ブロックの raw 出力が
    チャットに届かなかった。それを根拠に、引き継ぎメモへ「recon は一度も実行していない・
    原因は確定していない」と書こうとした。実際には Claude Code 側で実行済みで、
    再実行させたところ dev DB の実測とコード3段が揃い、原因は確定していた。
    Claude Code が上書き直前に停止して指摘し、誤った記録が焼き付く前に止まった。

    - 書けるのは「この会話に raw 出力が届いていない」までである。「未実行」「未確定」と書かない
    - 逆向きも同じ穴: 届いていない要約を「実測済み」として引き継ぎメモに転記するのも誤り
    - どちらも文言の調整では解けない。read-only の recon を再実行して
      raw 出力をチャットに通してから書く。コストはほぼゼロで、誤った記録のコストは大きい
    - これは鉄則8 後半（raw 出力が返らないことを未実行の根拠にしない）の再発である。
      鉄則として既に書いてあったのに、自分で破った

★「やってはいけないこと」に入れた
「✗ ★未証明の仮説を、報告や引き継ぎメモで『確定』『原因判明』『発見』と書く」の行も、
「✗ ★この会話に raw 出力が届いていないことを『未実行』『未確定』の根拠にする（鉄則4 / 鉄則8）」
に直す。

★description の「★未証明の仮説を『確定』『発見』と書かない。」も、
「★raw 出力が届いていないことを『未実行』の根拠にしない。」に差し替える。
...
- ★次セッションの順序: ⓪の保存確認 → ⑨のスキル修正 → ⑥の表の順
- ★B-192 は原因確定・修正未着手。⑥の1番として、④の (a)/(b) の選択から入る
