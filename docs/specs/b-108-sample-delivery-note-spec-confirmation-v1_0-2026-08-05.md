...
# 追補 v1.1（2026-08-08・PR #125 実装時の確定）

PR1b（PR #125）の実運用検証で判明した事項。本書 v1.0 の記載を一部改訂する。

## A-1. 編集機能の追加（§14 PR1 スコープの拡張）

**v1.0 は編集を明示要件にしていなかった**が、PR1b の検証で CRUD の U が
全層（ルート・action・validator・フォーム・詳細導線）で欠落していることが判明した。

- **§7 により削除した番号は再利用されない。** よって「作り直し」は DLV 番号を
  恒久的に焼却する。訂正のたびに番号が飛ぶ運用は成立しないため、編集は必須機能である。
- 追加したもの: `updateDeliveryNote` / `deliveries/[id]/edit/page.tsx` /
  詳細ページの編集導線 / `DeliveryNoteForm` の create/edit 兼用化。

### 編集可能範囲は DRAFT のみ（確定）

根拠は本書の既存思想との整合:
- §9 が論理削除を DRAFT 限定にしている（発行後は CANCELLED で残す）
- §4-3 が「発行後にマスターを修正しても発行済み伝票の宛先は変わらない」と定めている

発行済みを可変にすると §4-3 の snapshot 思想が崩れるため、編集も DRAFT に限定する。
非 DRAFT は詳細ページに編集ボタンを出さず、`[id]/edit` への直 URL 到達も
詳細へ redirect し、action 側でも弾く（UI とサーバの二重ガード）。

### 編集時の挙動

- **`deliveryNumber` は再採番しない**（保存済みの番号を保持）。
- **送り先は §4-2 の連鎖で再解決する。** 人の上書き入力があればそれを優先。
  DRAFT は未発行のため、再解決は §4-3（発行後は不変）に反しない。
- 明細は全削除→再作成（`DeliveryNoteItem` は `deletedAt` を持たず親従属。
  `updatePurchaseOrder` と同型）。

### 実装上の副次変更

検証・送り先解決・金額計算を共有ヘルパ `prepareDeliveryNote` に切り出し、
`createDeliveryNote` / `updateDeliveryNote` で共有した。create の挙動は不変。

## A-2. `showAmounts` の既定値を ON に改訂（§6 の改訂）

**§6 の「`showAmounts = false`（既定）」を「既定 ON」に改める。**

v1.0 執筆時は未運用のまま既定を決めていた。実運用では金額入りが基本であり、
伏せたい場合のみ外す運用が実態に合う。§6 のそれ以外の記述
（true/false それぞれの表示内容・単価の出所・税率）は変更しない。

## A-3. 単価未入力警告の実装位置（§6 の実装解釈）

§6 は「未入力の単価がある状態で `showAmounts = true` にした場合は保存時に警告
（ブロックしない）」と定める。当初はサーバ側 action が `warnings` を返し
フォームが保存後に表示する実装だったが、**保存直後の画面遷移と競合して
警告が表示されない**事象を確認した。

**保存前のクライアント側判定に移動した。**
- 保存を止めない点は §6 のとおり（警告のみ）。
- 業務上も、保存後に「単価が空だった」と知らされるより保存前に気づけるほうが有用。
- サーバ側の `warnings` 生成ロジックと `ActionResult` の `warnings` フィールドは
  温存している（将来サーバ由来の警告を増やす場合の受け皿）。

## A-4. 既知の制約（v1 で許容）

### 編集時に消費税率が 10% にリセットされる

`DeliveryNote` に税率を保持する列が無く、金額の確定値のみを持つ。
そのため編集画面の税率初期値は 10 固定になる。
**作成時に 8% 等を手入力した DRAFT を編集して保存すると、税額が 10% で再計算される。**

§6 が「v1 は 10% 固定＋手入力上書き可／軽減税率・輸出免税は B-109 の領分」と
定めているため v1 では許容する。税率の永続化が必要になった時点で
B-109（インボイス制度対応）と併せて設計する。

### 品番は明細で必須（§1 対象外の再確認）

§1 で「品番が決まる前に発生する見本の受け皿」を対象外とし、v1 は品番必須と確定した。
実運用の検証で、参考サンプルの売り立てなど**品番を登録しない納品**が
一定頻度で存在することが判明している。緩和には
`delivery_note_items.product_id` の `DROP NOT NULL`（migration・triple-gate）と
§1・§3-2 の改訂が必要なため、**B-121 として別途起票**する。本 PR では扱わない。

## A-5. 検証記録（2026-08-08 / dev = hopper:12921）

- 複数品番の混在（3品番・数量合計7）で作成・表示・編集が成立。
  §3-1 で `delivery_notes.product_id` を nullable 化した目的が実地で確認された。
- 送り先フォールバック（§4-2）が `Client` 基本住所まで到達することを確認。
  検証前に空欄だったのは dev のダミークライアントに住所が未入力だったためで、
  実装は v1.0 のとおり正しい。
- 採番は §7 のとおり論理削除行を母数に含み、番号は再利用されない
  （DLV-2026-0001 削除後、次は 0002）。
