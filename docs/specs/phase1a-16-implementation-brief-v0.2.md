...
`ExpenseCategory` モデル定義を以下の `CostCategory` に置換し、enum を改定する。ターゲットは仕様書 v0.2 §5 と同一。
...
- `DROP TABLE expense_categories`（または rename）+ `CREATE TABLE cost_categories`
- `DROP TYPE "ExpenseType"`、`"ExpenseCategoryStatus"` → `"CostCategoryStatus"` 化
- `expense_categories` 以外のテーブルに DROP/ALTER が出ていないこと
...
precedent（ProductCategory / MaterialCategory）の階層マスター実装を参照しながら、以下を改修。ディレクトリ・ファイル・シンボルを `expense`→`cost` / `Expense`→`Cost` にリネームしつつ、階層対応のロジックを追加する。
