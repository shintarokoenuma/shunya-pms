...
## v1.1 追補 (2026-06-12・慎太郎さん確認)

### Q1 追補: BOM の役割定義
BOM は品番直結の「現時点の正」1本とする。サンプルラウンドで資材が変わった場合は
BOM を書き換える (変更痕跡は AuditLog)。サンプルごとに実際何を使ったかの履歴は
SP 配下の PO (発注実績) が既に担っており、BOM を SP ごとに複製しない。
ラウンド別の構成凍結は、将来の仕様書 (Specification) バージョン連動 +
BOM バージョン管理 (QE-2・version 列予約済み) で実現する。

### Q2 追補: BomItem への品目属性3項目の追加 (QE-0c で実装)
発注書 (PoItem) との突き合わせの結果、「品目そのものの属性」は設計図である
BOM 側が持ち、PO へコピーされるべきと確定:
- supplierItemCode (仕入先品番)
- designNumber (デザイン番号)
- sizeSpec (サイズ/規格・文字列1本。数値化は B-039 の既定方針どおり)
取引固有の情報 (発注数量・納期・受入数量・確定金額・今回の発注先) は PO 側に残す。

### 入力フローの将来像 (B-057)
品番カルテで資材表を1回入力 (仕入先品番・サイズ含む)
→ サンプル発注: 資材表から PO 下書き生成 (数量は手入力)
→ 量産発注: 資材表 × 着数 × 取り切り計算 (QE-1) で PO 下書き生成
→ PO 側は数量・納期・確定単価の確認のみで発行
...
## v1.2 追補 (2026-06-13 確定)

### 正誤: BomItem 3項目の命名・構造統一
v1.1 §Q2 の BomItem 追加3項目を以下に変更（PoItem v1.1/v1.2 と命名・構造を統一）:
- designNumber → **designCode**
- sizeSpec（文字列1本）→ **sizeValue（Decimal）+ sizeUnit（cm/mm/m/inch）**
（supplierItemCode は変更なし。QE-0c で migration 29本目として ADD COLUMN×4 を実施）。

### 用語確定
- **着用尺** = 1着あたりの用尺。
- **総用尺** = 着数 × 着用尺（発注に必要な総量・QE-1 の出力）。
- **取り切り枚数** = 1反で取り切れる枚数（QE-1 の出力）。
- 「マーカー長」「マーカー内着数」「1着用尺」「総着数」は画面用語として使わない。

### MarkingRecord 仕様変更（A案・着用尺の根拠台帳）
着用尺はアプリが計算する数字ではなく、次の3択の入力値:
①直接指定 / ②マーキング図から人が読み取って入力 / ③将来 CAD 連携（B-047/B-056）。
- 換算機能（総用尺 ÷ 総着数 ÷ 100）は撤去。**着用尺は直接入力**（必須）。
- `totalUnits` / `totalLength` は nullable 化して**温存**（CAD 連携 B-047/B-056 用・UI 非対象）。
- `sizeCombination` は UI 撤去（**カラム温存**・何人取り等のコメントはメモへ集約）。
- `rollLength`（巻きメーター数・このロットの実規格）を追加。QE-1 取り切り計算で使用。
- 原本PDF は GCS 添付（B-053 基盤流用・`marking/{productId}/{JST}.pdf`・15分署名URLで閲覧）。
- BOM への転記は従来どおり「転記時点のコピー」（読み取り専用・実測バッジ・自動追従しない）。

### QE-1 設計論点（次回仕様確認の冒頭で詰める）
1. **計算モード2本立て**: 取り切り（巻きm → 取り切り枚数）/ 指定数（着数 → 総用尺）。
   デフォルトは指定数。サンプルは指定数が基本。
2. **規格の上書きは巻きメーター数のみ**（生地幅は変更不可の規格）。
3. 現場 Excel 2点（管理表・コスト表）の読み解き。
