...
# 追補 v1.3（2026-08-09・PR2b 実装で確定した仕様差分）

本追補は PR2b（PR #127・第1〜3段）の実装過程で確定した、v1.0/v1.1 からの
変更点を記録する。設計思想の変更ではなく、**実装で現物に当たった結果の確定**である。

## E-1. 「3タブ」→ **2タブ＋既存の行追加ボタン**

上位仕様 §5 は「タブ3: 手入力行の追加」としていたが、
**手入力による行追加は `delivery-note-form.tsx:354` に既存実装がある**
（PR1 で動作確認済み）。ダイアログ内に3つ目のタブを作ると同じ機能が
画面に2箇所並ぶため、**2タブ（サンプル／発注）とし、手入力は既存ボタンが担う**。

慎太郎さん確定 2026-08-09。既存の動くボタンを壊してまで字面に合わせる理由がない。

## E-2. 既定フィルタ「未納品のみ」→ **タブ1のみ適用**

§⑥ は「既定フィルタ = 未納品のみ」と定めていたが、判定に使える安定列は
`sourceSampleProductionId` のみ（§⑫）。WO/PO 明細は行 id が不安定なため、
**タブ2 では未納品フィルタが原理的に成立しない**。

- タブ1（サンプル）: `deliveredIn` に納品書番号を出す（安定・正確）
- タブ2（発注）: **全件表示＋親単位のバッジのみ**（`orderHasDelivery`）

これは §⑫「不安定な行 id でフィルタしてはならない」から導かれる帰結であり、
新たな判断ではなく明確化である。

## E-3. `blocked` を2種に拡張（§⑤ の拡張）

v1.0 §⑤ は「親の品番が null の明細」のみを警告対象としていたが、
**品番が物理削除されて存在しないケース**が実在する（2026-08-09 recon で
dev に発生済み）。黙って除外すると §⑤ の趣旨（引き当てられない発注が
あることを気付かせる）に反するため、警告対象に含める。

| reason | 意味 | 対処文言 |
|---|---|---|
| `NO_PRODUCT` | 発注に品番が紐づいていない | 発注側で品番を設定してください |
| `PRODUCT_MISSING` | 品番を指しているが Product が存在しない | 品番が削除されている可能性があります。発注側で品番を設定し直してください |

UI では**2セクションに分けて常時表示**（折りたたまない）。

なお、削除側の予防（参照内訳を見せて削除を躊躇させる）は **B-126** として起票。
上流で止め、下流で気づく二層構造とする。

## E-4. 候補一覧は**品番でグルーピング**（1階層・ブランド名併記）

フラットな一覧では明細の切り替わりが判別しづらいという指摘（慎太郎さん
2026-08-09）を受け、**品番ごとに見出しを立てて束ねる**。

    [AOI] AOI-26SS-M-TS-001  スポットプリントTシャツ
        ☐ ブロード      PO-2026-0001  3m
        ☐ 検証明細      WO-2026-0002  ¥30,000

- ブランド階層は作らない（1階層）。ブランド名は見出しに `[AOI]` として併記
- ソートは brandName（null は末尾）→ productCode
- **絞り込みフィルタは付けない**。既定で品番に絞ると §⑥ の漏れ検知が死ぬため
- `Product` に `brand` relation は無い（scalar FK・house style）ため、
  ブランド名は `Brand` を別クエリで引いて Map で解決する

## E-5. サンプル引き当て時の品名は `title` 優先

同一品番の複数サンプルを引き当てると、品番名を使う実装では明細上で
区別できない（実測: SP-2026-0004 と 0005 が両方「スポットプリントTシャツ」）。

`SampleProduction.title` があればそれを品名に使い、無ければ品番名にフォールバック。

## E-6. `SampleProduction` は SKU / 色 / サイズを持たない（recon 確定）

2026-08-09 recon: `SampleProduction` / `SampleRevision` に sku / color / size 列は
無く、samples 配下の UI も SKU を参照していない。`Sku` は `colorwayId` NOT NULL
＋ `size` NOT NULL のため、色もサイズも未定のサンプル段階では紐づける先が無い。

→ **引き当て時に色・サイズを自動補完する経路は存在しない**（空欄で人が入力する
現行が正しい）。サンプルに色・サイズを持たせる設計は **B-127** として起票。

## E-7. 実装上の確定事項（recon 実測）

- `WoItem` → `WorkOrder` の relation 名は **`wo`**（`workOrder` ではない）
- `PoItem` → `PurchaseOrder` の relation 名は **`po`**
- `delivery-note-form.tsx` は `useFieldArray` ではなく **`useState` 配列管理**
- `src/components/ui/tabs.tsx` は**存在しない**（自前のボタン切替で実装）
- PO 明細の物理資産チェック ON 時、**保管開始日・期限が必須**（実測）

## E-8. round-trip 実測（§C-3 の検証結果）

dev `DLV-2026-0005` で実測（2026-08-09）:

| 行 | 引き当て元 | セットされた列 |
|---|---|---|
| 0 | サンプル | `sourceSampleProductionId` のみ |
| 1 | PO（版代） | `sourcePoItemId` ＋ `sourcePurchaseOrderId` |

種別ごとに正しい列だけが入り、無関係な列は null。
編集 → 無変更保存を越えて残存することを確認。**§C-3 の要件を満たす。**

## E-9. 改訂履歴（追記）

| 日付 | バージョン | 内容 |
|---|---|---|
| 2026-08-09 | v1.3 | PR2b 実装で確定した差分を記録。3タブ→2タブ、未納品フィルタはタブ1のみ、blocked を2種に拡張（PRODUCT_MISSING 追加）、品番グルーピング（1階層）、サンプル品名は title 優先、SampleProduction は SKU を持たない旨を確定。B-126 / B-127 を起票 |
