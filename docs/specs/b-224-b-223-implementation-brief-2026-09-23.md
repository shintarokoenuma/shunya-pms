...
| D-50 | 検知の定義: そのクライアントの入金（D-25 の集計条件）のうち、**入金日が今回の窓の始まりより前**で、かつ**その入金日を窓に含む請求書が無いか、あってもその請求書が入金より先に作られた**（`invoice.createdAt < payment.createdAt`）もの | D-39 の原文「発行済みの請求書より後に、その請求書の期間内の日付で記録された入金は、どの請求書にも現れない」をそのまま判定に落としたもの |
...
2. そのクライアントの取消されていない請求書を `periodEndDate` 昇順で取り、各請求書の窓 `[前の periodEndDate + 1日, 自分の periodEndDate]` と `createdAt` を作る（最初の1枚の窓の始まりは `-infinity` 相当として扱う）
3. 各入金 P について:
   - `P.paymentDate >= paymentWindow.start` なら**今回計上される**ので除外
   - `P.paymentDate` を窓に含む請求書 I を探す
   - I が無い → **未計上**
   - I があり `I.createdAt < P.createdAt` → **未計上**（発行後に記録された）
...
## 2-5. dev での確認（素材は今日作ったものがそのまま使える）

dev には `INV-2026-0001`（葵アパレル・2026-09）と、**その後に記録した 8/31 の入金 `PAY-2026-0003`（5,000）** がある。これが D-39 そのものの形。

1. `/invoices/new` で 葵アパレル・**2026-10-01 〜 2026-10-31** を選ぶ
2. 右カラムに警告が出て、`PAY-2026-0003　2026/08/31　¥5,000` が並ぶ
3. 期間を 2026-08-01 〜 2026-08-31 にすると、その入金は**今回計上される**ので警告が消える

★確認の前に、dev の請求書と入金の現状を read-only で測り直す（今日のダミーを消していないか）。
