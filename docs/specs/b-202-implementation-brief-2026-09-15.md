...
> ★2026-09-16 追記: **本書の §2〜§5（PR-1〜PR-4 の内容）は上書きされている。**
> PR-1 は `addendum v0.3`（MOCK-C）で作り直し、PR-2〜PR-4 は `addendum v0.4`（PR1R-DONE）§3 で再定義した。
> 本書で今も有効なのは §0（着手前の実測）・§1 の PR 分割の考え方・§6 以降の共通ルールのみ。
> ★設計の正は v1.0 ＋ addendum v0.1 / v0.2 / v0.3 / v0.4 の5本セットで、最上位は **v0.3 §0 のモック**。
