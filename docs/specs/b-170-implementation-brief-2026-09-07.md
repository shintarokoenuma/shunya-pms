...
**未入力警告（v1.0 §6-2）を本PRに含めるか、B-171 と同時にするか。**

v1.0 §6-2 は「管理側の未入力警告は品番だけでなく配色についても出す」と確定している。ただし置き場所と時期は決めていない。

- 本PRに含める … 先方色名を入れ始めるのは今日からなので、入れ忘れの検出が早いほど溜まらない。実装は小さい（一覧に未登録件数を出す程度）
- B-171 と同時にする … 警告が効くのは「先方ページに出ない」という結果があってこそ。先方ページが無い今は、警告を見ても対処の締め切りが無い

★どちらにする場合も、**本PRのスコープ外にするなら B-171 の BACKLOG 定義欄に追記する**（新規起票はしない。roadmap-audit 提案2 の規律を、番号を増やさずに満たす）。
