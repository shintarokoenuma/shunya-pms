...
| P4-D21 | **品番は途中で折り返さない**（2026-09-26 試し刷りで発見: 「AOI-26SS-M-TS-00 / 1」）。品番の Text は語を割らない hyphenationCallback（NO_BREAK）。納品書は品番の列を 22% に広げ、請求書は品番／品名の欄を2段（上に品番を小さく）にする。色名など他の値は折り返してよい（ハイフンは入れない・P4-D19） |
...
4. **明細の表**: 列は 日付（mm/dd）／伝票番号／品番 / 品名／色・サイズ／数量／単価／金額
   - 納品の行: 日付＝納品日・伝票番号＝納品書番号・上段に {itemCode}（小さく）・下段に {itemName}・「{色名}・{サイズ}」
...
## §9 実装後の追記（2026-09-26 締め CLOSE-AA）

- P4-D21 の補足: 納品書の品名セルでは、品名の Text に `flex: 1` を付けた。付けないと長い品名が「前受金充当」の札に重なる（dev で実測。これまで commit 本文にだけ書いていたのをここへ移す）
- 本番はマージ直後の時点で納品書 0 件・請求書 0 件。本番の紙面は未確認で、最初の実データで確認する
- §8 の行き先の B番号: 設定ページ → B-205 に追記／受領スキャン → B-113 に追記／複数まとめて PDF → B-238（新規）／英語版 → B-110／締め → B-123
