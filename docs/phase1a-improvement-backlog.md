...
- **記録日**: 2026-06-10
- **記録者**: Shin（肥沼慎太郎） + Claude
- **優先度**: 中〜将来（業務トランザクション一巡後）
- **目安タイミング**: S-4 完了後、量産フェーズ着手時
...
- 既存: `Material.standardUsage`（標準用尺）/ `Material.standardLossRate`（標準ロス率）/ `Material.fabricWidth`（生地幅）。
- **不足**: 原反長（roll length）列が未存在。見積エンジン設計時に migration で追加する。
- 前提タスク: B-039（用尺の数値構造化）。

#### 関連

- B-038（見積→量産マスター化候補提示）/ B-047 / B-044（オーナー指標：見積 vs 実績）。
- 実績側は S-4c-1（コスト集計）で整備済み。

---

### B-047: CAD 連携の用尺見込み計算

- **記録日**: （既存項目・SESSION_HANDOVER 継続バックログより。本ファイルへ転記）
- **記録者**: Shin（肥沼慎太郎） + Claude
- **優先度**: 将来
- **目安タイミング**: 見積もりエンジン（QE）成熟後

#### 内容

CAD 連携による用尺見込み計算。**東レ CAD 等との連携でマーキングを取得するところまでを視野**に入れる（QE-0 で新設する `MarkingRecord.source` に `CAD` を予約済み）。
...
- **記録日**: 2026-06-12
- **記録者**: Shin（肥沼慎太郎） + Claude
- **優先度**: 将来（Phase 1E の AI 機能）
- **目安タイミング**: QE-0/QE-1 稼働後、原本添付データが蓄積してから

#### 内容

マーキング図 PDF / パターンデータをアップロード → 項目（生地幅・総着数・総用尺・収率・パーツ数・柄ピッチ等）を AI 抽出 → 確認画面 → `MarkingRecord` 化する自動読み取り。**QE-0 の `MarkingRecord` + 原本 PDF 添付（GCS）が基盤**で、v1 から原本を保存するため教師データが自然に蓄積される。
...
- **記録者**: Shin（肥沼慎太郎） + Claude
- **優先度**: 中（QE-1 と前後して実装）
- **目安タイミング**: QE-0c（BomItem 3項目追加）/ QE-1（取り切り計算）の後

#### 内容

品番カルテの資材表（BOM）を 1 回入力すれば、発注（PO）下書きを自動生成する入力フロー:

1. 品番カルテで資材表を入力（仕入先品番・サイズ含む = QE-0c で BomItem に追加する3項目）。
2. **サンプル発注**: 資材表から PO 下書きを生成（数量は手入力）。
3. **量産発注**: 資材表 × 着数 × 取り切り計算（QE-1）で PO 下書きを生成。
4. PO 側は数量・納期・確定単価の確認のみで発行。

「品目そのものの属性」は設計図である BOM が持ち PO へコピー、取引固有情報（数量・納期・受入・確定金額・発注先）は PO 側に残す、という QE-0 v1.1 の役割分担に基づく。
...
資材表（BOM）に「発注から取り込む」ボタンを置き、品番に紐づく PO 一覧から選択 → 明細をチェックボックスで選んで BomItem へ転記・修正する。サンプル段階の二重入力（PO で入れた資材を BOM にも手入力）を解消する。

- **B-057（BOM→PO・量産方向）と対**で双方向（PO→BOM=サンプル実績の取り込み / BOM→PO=量産の下書き生成）。
- 自動同期はしない（**転記時点コピー + 人の確認**の原則）。
...
- **記録者**: Shin（肥沼慎太郎） + Claude
- **優先度**: 中〜将来（業務トランザクション一巡後）
- **目安タイミング**: BOM / PO / QE-1（部品）が揃ってから

#### 内容

サンプル品番 → 量産品番への複製（BOM・マーキング実測等を引き継ぐ）。量産時は大きな変更はなく、修正・用尺調整・数量（+ 取り切り/指定数のチョイス）あたりが中心、という業務実態に基づく「**転記ファースト原則**」の幹。

- B-025（量産パターン管理）と**仕様確認をセット**で行う。
- 実装は業務トランザクション一巡の中で（部品 = BOM / PO / QE-1 が揃ってから）。
