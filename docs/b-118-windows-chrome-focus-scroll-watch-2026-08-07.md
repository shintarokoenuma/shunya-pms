...
### 追加観測（2026-08-08・ブラウザ拡張が DOM に介入する実例）

Mac Chrome の dev 環境で、Next.js のハイドレーション不一致エラーを確認した。
原因は Feedly Mini 拡張が `<body>` に `data-feedly-mini="yes"` を注入していたことで、
アプリ側のコードに非はない（Next.js のエラーメッセージ自身も
「ブラウザ拡張が React 読み込み前に HTML をいじった場合にも起きる」と明示している）。

**B-118 にとっての意味**: ブラウザ拡張が DOM に実際に介入している実例が取れた。
B-118 が (a) Windows Chrome 限定 (b) 再読み込みで消える (c) コードに非が無い、
という性質と噛み合う。Mac と Windows で導入されている拡張の構成は異なるため、
Windows 側の拡張が引き金である可能性を補強する材料になる。

→ **次回再発時の確認手順に追加**: §5 の計測スニペットを取ったうえで、
   **シークレットウィンドウ（拡張無効）で再現するかを必ず確認する。**
   再現しなければ拡張が原因と確定でき、そこで調査は終了できる。
