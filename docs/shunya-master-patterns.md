...
**バージョン**: v1.2 (ライブドキュメント、Phase 進行中も随時更新)
**最終更新**: 2026年5月31日 (Phase 1A-17 完了、§9 にコードサジェスト共通化を反映)
**確立元**: Phase 1A-1 〜 Phase 1A-17
...
### 基本情報カードの並び順（Phase 1A-17 で標準化）

**マスターフォームの基本情報カードは、必ず「名称を先頭、コードを次」に置く。**

- カテゴリ系マスター (商品 / 素材 / 原価費目): 名称 → コード。コード入力欄の直下に `<CodeSuggester dict={...} />` を配置し、入力中の名称 + 親コードから候補を提示する。
- 連絡先系マスター (Supplier / Factory / Contractor / Client / Buyer / DeliveryDestination / Material 等): 名称 → コード。**サジェスター無し** (コードは運用ルールに従う手入力、採番ルールを尊重)。
- 自動採番のマスター (ModelCode 等) は対象外 (コード入力欄自体を持たない)。

### 一覧テーブルの列順（Phase 1A-17 で標準化）

**マスター一覧テーブル (`*-table.tsx`) の列順も同じく「名称をコードより左」に置く。**

- すべてのマスター一覧で、左から「名称」→「コード」の順 (ヘッダー `<TableHead>` と本体 `<TableCell>` をセットで揃える)。
- ただし `cost-categories` のような「階層 (Lv)」列を持つ一覧は階層を先頭に保持: 階層 → 名称 → コード → ... の順。
- 詳細ページへの遷移リンク (`<Link>`) はそのセルに乗っているまま列ごと移動する。フォームの並び順 (上記) と一致させる。

### コードサジェスト基盤 (B-007 回収)

| レイヤ | パス | 役割 |
|---|---|---|
| 汎用関数 | `src/lib/utils/code-suggest.ts` | `suggestCodeFromName(name, parentCode, dict, maxResults?)` |
| ドメイン辞書 | `src/lib/constants/code-dicts/apparel.ts` | `APPAREL_TERM_DICT` (ProductCategory 用) |
| | `src/lib/constants/code-dicts/material.ts` | `MATERIAL_TERM_DICT` (MaterialCategory 用) |
| | `src/lib/constants/code-dicts/cost.ts` | `COST_TERM_DICT` (CostCategory 用) |
| 共通 UI | `src/components/code-suggester.tsx` | `<CodeSuggester>` |

旧 utility (`category-code-suggest.ts` / `material-category-code-suggest.ts`) と旧サジェスター UI (`category-code-suggester.tsx` / `material-category-code-suggester.tsx`) は、後方互換のため薄い re-export / ラッパーとして残置。
...
ProductCategory / CostCategory 等。シンプルなフォーム構成：

1. 基本情報（**名称 → コード** の順、コード欄直下に共通 `<CodeSuggester>`）
2. 分類（マスター固有 enum、親階層）
