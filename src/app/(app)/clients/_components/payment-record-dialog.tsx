...
/**
 * addendum v0.9 §2-4: 「入金を記録」。項目は 入金日 / 金額 / 方法 / 摘要 の4つ。
 * 内部では予定日＝入金日、状態＝着金確認済みで保存する（実績の記録）。請求書への紐付けはしない（D-25）。
 * B-222 PR-2d ブリーフ §4-7: props はどちらか一方。
 * - clientId 固定（クライアント詳細から）
 * - clients 選択（/payments から）: クライアントの select を最初の項目に出し、未選択なら記録できない。
 *   記録に成功しても選んだクライアントは初期化しない（同じ取引先の入金を続けて入れることがある）。
 */
type PaymentRecordDialogProps =
  | { clientId: string; clients?: never }
  | { clientId?: never; clients: { id: string; companyName: string }[] }

export function PaymentRecordDialog(props: PaymentRecordDialogProps) {
  ...
  const selectable = props.clients !== undefined
  const clientId = selectable ? selectedClientId : props.clientId
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            {selectable ? "入れるのはこの5つだけです。" : "入れるのはこの4つだけです。"}
            内部では予定日＝入金日、状態＝着金確認済みで保存します（実績の記録なので予定と実績が同じ）。請求書への紐付けはしません。
          </p>
        </div>
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={isPending || !amount || !paymentDate || !clientId}>
