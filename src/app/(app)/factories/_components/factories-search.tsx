...
export function FactoriesSearch() {
  ...
  const [factoryType, setFactoryType] = useState(
    searchParams.get("factoryType") ?? "_all"
  )
  ...
  const apply = (next: {
    ...
    setParam("factoryType", next.factoryType ?? factoryType)
    params.delete("page")
    startTransition(() => {
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-wrap items-end gap-3">
      ...
      <div className="w-[180px]">
        <label className="text-xs text-muted-foreground mb-1 block">工場タイプ</label>
        <Select
          value={factoryType}
          onValueChange={(v) => {
            setFactoryType(v)
      ...
      <div className="w-[140px]">
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="_all">すべて</SelectItem>
            {Object.entries(FACTORY_STATUS_LABELS).map(([k, v]) => (
