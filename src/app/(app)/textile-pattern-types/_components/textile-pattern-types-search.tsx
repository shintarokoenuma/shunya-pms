...
import { TEXTILE_PATTERN_TYPE_STATUS_LABELS } from "./labels"

export function TextilePatternTypesSearch() {
  ...
  const apply = (next: { q?: string; status?: string }) => {
    ...
    params.delete("page")
    startTransition(() => {
      router.push(`/textile-pattern-types?${params.toString()}`)
    })
  }
  ...
  const onReset = () => {
    ...
    setStatus("_all")
    startTransition(() => {
      router.push("/textile-pattern-types")
    })
  }
      ...
      <div className="w-[140px]">
        ...
        >
          ...
          <SelectContent>
            ...
            {Object.entries(TEXTILE_PATTERN_TYPE_STATUS_LABELS).map(
              ([k, v]) => (
