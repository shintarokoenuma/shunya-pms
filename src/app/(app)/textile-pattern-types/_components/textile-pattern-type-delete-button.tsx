...
import {
  archiveTextilePatternType,
  restoreTextilePatternType,
  deleteTextilePatternTypePermanently,
  checkTextilePatternTypeUsage,
  type TextilePatternTypeUsage,
...
export function TextilePatternTypeActions({
  id,
  name,
  status,
  isMasterAdmin,
  ...
  const [usage, setUsage] = useState<TextilePatternTypeUsage | null>(null)
  ...
  useEffect(() => {
    if (mode === "delete") {
      ...
      setUsage(null)
      checkTextilePatternTypeUsage(id)
        .then(setUsage)
  ...
  const onArchive = () => {
    startTransition(async () => {
      const result = await archiveTextilePatternType(id)
      if (!result.ok) {
        toast.error(result.error)
        return
      }
      toast.success("柄種別をアーカイブしました")
      close()
  ...
  const onRestore = () => {
    startTransition(async () => {
      const result = await restoreTextilePatternType(id)
      if (!result.ok) {
        toast.error(result.error)
        return
      }
      toast.success("柄種別を復元しました")
      close()
      if (onChanged) onChanged()
      else router.refresh()
    })
  ...
  const onDelete = () => {
    startTransition(async () => {
      const result = await deleteTextilePatternTypePermanently(id, confirmName)
      if (!result.ok) {
        toast.error(result.error)
        return
      }
      toast.success("柄種別を本削除しました")
      close()
      if (onChanged) onChanged()
      else router.push("/textile-pattern-types")
      router.refresh()
    })
  }
  ...
  const trigger = (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        {variant === "icon" ? (
          <Button variant="ghost" size="icon" aria-label="柄種別 操作">
  ...
  return (
    <>
      ...
      <Dialog open={mode === "archive"} onOpenChange={(o) => !o && close()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>柄種別をアーカイブしますか？</DialogTitle>
            <DialogDescription>
      ...
      <Dialog open={mode === "restore"} onOpenChange={(o) => !o && close()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>柄種別を復元しますか？</DialogTitle>
            <DialogDescription>
              「{name}」を稼働中に戻します。
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <div className="text-sm">
                  確認のため、柄種別名「<strong>{name}</strong>」を入力してください：
                </div>
                <Input
                  value={confirmName}
