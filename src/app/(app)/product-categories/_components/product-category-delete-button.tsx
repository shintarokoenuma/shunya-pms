...
import {
  ...
  deleteProductCategoryPermanently,
  checkProductCategoryUsage,
...
export function ProductCategoryActions({
  ...
  const onArchive = () => {
    startTransition(async () => {
      const result = await archiveProductCategory(id)
      if (!result.ok) {
        toast.error(result.error)
        return
      }
      toast.success("商品カテゴリをアーカイブしました")
      close()
  ...
  const onRestore = () => {
    startTransition(async () => {
      const result = await restoreProductCategory(id)
      if (!result.ok) {
        toast.error(result.error)
        return
      }
      toast.success("商品カテゴリを復元しました")
      close()
      if (onChanged) onChanged()
      else router.refresh()
    })
  ...
  const onDelete = () => {
    startTransition(async () => {
      const result = await deleteProductCategoryPermanently(id, confirmName)
      if (!result.ok) {
        toast.error(result.error)
        return
      }
      toast.success("商品カテゴリを本削除しました")
      close()
      if (onChanged) onChanged()
  ...
  return (
    <>
      ...
      <Dialog open={mode === "archive"} onOpenChange={(o) => !o && close()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>商品カテゴリをアーカイブしますか？</DialogTitle>
            <DialogDescription>
      ...
      <Dialog open={mode === "restore"} onOpenChange={(o) => !o && close()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>商品カテゴリを復元しますか？</DialogTitle>
            <DialogDescription>
              「{name}」を稼働中に戻します。
