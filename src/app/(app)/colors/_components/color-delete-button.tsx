...
export function ColorActions({
  ...
  const onArchive = () => {
    startTransition(async () => {
      const result = await archiveColor(id)
      if (!result.ok) {
        toast.error(result.error)
        return
      }
      toast.success("色をアーカイブしました")
      close()
  ...
  const onRestore = () => {
    startTransition(async () => {
      const result = await restoreColor(id)
      if (!result.ok) {
        toast.error(result.error)
        return
      }
      toast.success("色を復元しました")
      close()
      if (onChanged) onChanged()
      else router.refresh()
    })
  ...
  const onDelete = () => {
    startTransition(async () => {
      const result = await deleteColorPermanently(id, confirmName)
      if (!result.ok) {
        toast.error(result.error)
        return
      }
      toast.success("色を本削除しました")
      close()
      if (onChanged) onChanged()
      else router.push("/colors")
      router.refresh()
    })
  }
  ...
  const trigger = (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        {variant === "icon" ? (
          <Button variant="ghost" size="icon" aria-label="色 操作">
  ...
  return (
    <>
      ...
      <Dialog open={mode === "archive"} onOpenChange={(o) => !o && close()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>色をアーカイブしますか？</DialogTitle>
            <DialogDescription>
      ...
      <Dialog open={mode === "restore"} onOpenChange={(o) => !o && close()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>色を復元しますか？</DialogTitle>
            <DialogDescription>
              「{name}」を稼働中に戻します。
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <div className="text-sm">
                  確認のため、色名「<strong>{name}</strong>」を入力してください：
                </div>
                <Input
                  value={confirmName}
