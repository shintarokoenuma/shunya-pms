...
import {
  ...
  deleteMaterialCategoryPermanently,
  checkMaterialCategoryUsage,
  type MaterialCategoryUsage,
...
export function MaterialCategoryActions({
  id,
  name,
  status,
  isMasterAdmin,
  ...
  const [usage, setUsage] = useState<MaterialCategoryUsage | null>(null)
  ...
  const onArchive = () => {
    startTransition(async () => {
      const result = await archiveMaterialCategory(id)
      if (!result.ok) {
        toast.error(result.error)
        return
      }
      toast.success("素材カテゴリをアーカイブしました")
      close()
  ...
  const onRestore = () => {
    startTransition(async () => {
      const result = await restoreMaterialCategory(id)
      if (!result.ok) {
        toast.error(result.error)
        return
      }
      toast.success("素材カテゴリを復元しました")
      close()
      if (onChanged) onChanged()
      else router.refresh()
    })
  ...
  const onDelete = () => {
    startTransition(async () => {
      const result = await deleteMaterialCategoryPermanently(id, confirmName)
      if (!result.ok) {
        toast.error(result.error)
        return
      }
      toast.success("素材カテゴリを本削除しました")
      close()
      if (onChanged) onChanged()
      else router.push("/material-categories")
      router.refresh()
    })
  }
  ...
  const trigger = (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        {variant === "icon" ? (
          <Button variant="ghost" size="icon" aria-label="カテゴリ操作">
  ...
  return (
    <>
      ...
      <Dialog open={mode === "archive"} onOpenChange={(o) => !o && close()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>素材カテゴリをアーカイブしますか？</DialogTitle>
            <DialogDescription>
              「{name}」をアーカイブします。一覧から「稼働中」では非表示になりますが、
              「アーカイブ」絞り込みで再表示・復元が可能です。
              稼働中の子カテゴリが存在する場合はアーカイブできません。
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      <Dialog open={mode === "restore"} onOpenChange={(o) => !o && close()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>素材カテゴリを復元しますか？</DialogTitle>
            <DialogDescription>
              「{name}」を稼働中に戻します。
              親カテゴリがアーカイブされている場合は復元できません。
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={close} disabled={isPending}>
            ...
            <div className="rounded-md border border-destructive bg-destructive/10 p-3 text-sm">
              ...
              <ul className="list-disc list-inside mt-1 text-foreground">
                {usage.childrenCount > 0 && (
                  <li>子カテゴリ: {usage.childrenCount} 件</li>
                )}
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <div className="text-sm">
                  確認のため、カテゴリ名「<strong>{name}</strong>」を入力してください：
                </div>
                <Input
                  value={confirmName}
