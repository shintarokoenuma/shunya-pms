...
import {
  archiveContractor,
  restoreContractor,
  deleteContractorPermanently,
  checkContractorUsage,
...
export function ContractorActions({
  ...
  const [usage, setUsage] = useState<ContractorUsage | null>(null)
  ...
  const onDelete = () => {
    startTransition(async () => {
      const result = await deleteContractorPermanently(id, confirmName)
      if (!result.ok) {
        toast.error(result.error)
        return
      }
      toast.success("外注先を本削除しました")
      close()
      if (onChanged) onChanged()
      else router.push("/contractors")
      router.refresh()
    })
  }
  ...
  const trigger = (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        {variant === "icon" ? (
          <Button variant="ghost" size="icon" aria-label="外注先操作">
  ...
  return (
    <>
      ...
      <Dialog open={mode === "archive"} onOpenChange={(o) => !o && close()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>外注先をアーカイブしますか？</DialogTitle>
            <DialogDescription>
      ...
      <Dialog open={mode === "restore"} onOpenChange={(o) => !o && close()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>外注先を復元しますか？</DialogTitle>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <div className="text-sm">
                  確認のため、外注先名「<strong>{name}</strong>」を入力してください：
                </div>
                <Input
                  value={confirmName}
