...
/**
 * B-085 量産見積 見積書 PDF（QE-1R 道A のスタイル踏襲・PE 専用）。
 * QE-1R ドキュメントとは共通化しない（セクション意味論が異なるため・spec §B）。
 * 総合計＝製品合計＋別枠合計（spec §3・消費税レイヤーなし）。
 */
...
const styles = StyleSheet.create({
  ...
  summaryBox: { marginTop: 14, marginLeft: "auto", width: "50%" },
  grandRow: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "baseline",
        ...
        <View style={styles.table}>
          ...
          {data.productRows.map((r, i) => (
            <View style={styles.tr} key={i} wrap={false}>
              ...
              <Text style={styles.cUnit}>{yen(r.unitPriceJpy)}</Text>
              <Text style={styles.cAmount}>{yen(r.amountJpy)}</Text>
            </View>
          ))}
          <View style={styles.totalRow}>
            <Text style={styles.totalLabelCell}>製品合計</Text>
            <Text style={styles.totalValueCell}>{yen(data.productTotalJpy)}</Text>
          </View>
        </View>

        {/* 【初期費用（別途）】 行が0件ならセクションごと非表示 */}
        {data.separateRows.length > 0 && (
          <>
            ...
            <View style={styles.table}>
              ...
              <View style={styles.totalRow}>
                <Text style={styles.totalLabelCell}>別枠合計</Text>
                <Text style={styles.totalValueCell}>
                  {yen(data.separateTotalJpy)}
                </Text>
              </View>
            </View>
          </>
